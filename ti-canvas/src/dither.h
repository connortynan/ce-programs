...
static void dither_fillBayerMatrix(int16_t* matrix, uint8_t subDim, uint8_t dim) {
    if (subDim < 1) {
        return;
    }
    if (subDim == 1) {
    ...
    uint8_t sub = subDim / 2;
    dither_fillBayerMatrix(matrix, sub, dim);
...
static void dither_generateDitherPattern(int16_t* pattern, uint8_t dim, uint8_t ditherStrength) {
    ...
    dither_fillBayerMatrix(pattern, dim, dim);
